        ...
        try:
            with conn.cursor() as cur:
                if truncate:
                    ...
                    if db_min_time is not None and df_min_time > db_min_time:
...
def upload_stock_DataFrame_with_polars(dfs: list, value_names: list, db_user: str, db_password: str, local_host=False, table_name: str = "adjusted_time_series_data_stock", truncate=False):
    """
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                if not is_hypertable:
                    cur.execute(f"SELECT create_hypertable('{table_name}', 'time', if_not_exists => TRUE);")
                    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_{table_name}_sedol ON {table_name} (sedol);")
                conn.commit()
        finally:
            conn.close()

...
이므로, 요약 출력 한 줄 때문에 방금 받아온 패널 전체가 csv 로 앉지 못하고 통째로 날아간다.

**비용 정정 (2026-08-19 실측)**: 처음 이 note 를 쓸 때는 "수십 분짜리 pull 이 날아간다" 고 적었는데,
그날 관측한 87분 실행은 병렬 실행 2개가 DB 를 포화시킨 비정상 상황이었다. 유휴 상태에서 전체이력
(SPY+QQQ, 2000-01~) 을 돌리니 **총 53초** 였다 (가격 5,387,592행 = 16.6초). 즉 유실 비용은 1분
남짓이고, 이 note 의 severity 는 데이터 유실의 크기가 아니라 **crash 가 조용하다는 점**에 있다.
